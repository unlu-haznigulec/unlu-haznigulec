...
//model portfolio hisse seçim
@RoutePage()
class ModelPortfolioOrderPage extends StatefulWidget {
  final String title;
  final List<QuickPortfolioAssetModel> portfolioList;

  const ModelPortfolioOrderPage({
    super.key,
    required this.title,
  ...
  @override
  State<ModelPortfolioOrderPage> createState() => _ModelPortfolioOrderPageState();
}

class _ModelPortfolioOrderPageState extends State<ModelPortfolioOrderPage> {
  final TextEditingController _amountController = TextEditingController(text: MoneyUtils().readableMoney(5000));
  double _amount = 5000;
  List<QuickPortfolioAssetModel> _selectedAssets = [];
  bool _showDefault = true;
  bool _isFocus = false;
  String _selectedAccount = '';
  double _cashBalance = 5000;
              ...
              child: Column(
                children: [
                  Padding(
                    ...
                    child: Text(
                      L10n.tr(
                        'analysis_model_portfolio_description',
                        args: [
                          MoneyUtils().readableMoney(5000),
                        ],
                      ),
  ...
  _doContinue() {
    ...
    if (MoneyUtils().fromReadableMoney(price) < 5000) {
      return PBottomSheet.showError(
        context,
